...
async function yt(url, quality, type, bitrate, server = 'en68') {
  ...
  switch (type) {
    case 'mp4':
      list = Object.fromEntries([...table.querySelectorAll('td > a[href="#"]')].filter(v => !/\.3gp/.test(v.innerHTML)).map(v => [v.innerHTML.match(/.*?(?=\()/)[0].trim()]))
      break
    case 'mp3':
      list = {
        '128kbps': table.querySelector('td > a[href="#"]')
      }
      break
    default:
      list = {}
